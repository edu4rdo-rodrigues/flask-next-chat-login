// flask-next-chat-login/frontend/components/Login.tsx
...
export const Login: React.FC<ILoginProps> = ({ setIsLogged, setUser, user }) => {
  ...
  /*const redirectChat = async () => {
    if (user) {
      ...
      await axios.get(`${API_BACKEND_URL}/user/${user.id}`)
      .then(response => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      await axios.post(`${API_BACKEND_URL}/login`, data)
      .then(response => {
        if (response.status === 200) {
